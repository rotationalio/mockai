...
router.post("/v1/completions", async (req, res) => {
  ...
  if (serverDown()) {
    requestCounter.inc({ method: "POST", path: "/v1/completions", status: 500 });
    requestLatency.observe({ method: "POST", path: "/v1/completions", status: 500 }, (Date.now() - then));
    payloadSize.observe({ method: "POST", path: "/v1/completions", status: 500 }, req.socket.bytesRead);
